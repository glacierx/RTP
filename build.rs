    ...
    pub fn from_xml_element(element: Element) -> Result<Self, String> {
        ...
        for child in element.children {
            ...
            if let XMLNode::Element(child_elem) = child {
                let id = child_elem.attributes.get("id")
                    .ok_or(String::from("no id attribute in one of the child"))?
                    .to_owned();
                
                let value_string = child_elem.attributes.get("value")
                ...
                let prompt = child_elem.attributes.get("prompt")
...
fn generate_error(input_xml: &Path, output_rs: &Path) -> Result<(), String> {
    ...
    input_file.read_to_end(&mut file_bytes)
    ...
    let mut error_output = std::io::BufWriter::new(
        std::fs::File::create(output_rs)
            .map_err(|e| format!("cannot create error file, {}", e))?
    );

    for error in errors.errors.iter() {
        error_output
            .write(format!("pub const ERROR_{}: TThostFtdcErrorIDType = {};\n", 
                error.id, error.value).as_bytes())
    ...
    error_output.write(b"        _ => \"unknown error\",\n")
        .map_err(|e| format!("cannot write error file, {}", e))?;
...
fn generate_bindings(api_type: &str, header_file: &str, out_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut output_file = std::fs::File::create(out_path).map_err(|e| format!("cannot create struct file, {}", e))?;
    output_file.write_all(binding_output.as_bytes()).map_err(|e| format!("cannot write struct file, {}", e)).unwrap();    
    Ok(())
}
...
fn setup_api(api_type: &str, so_name: &str, orig_name: &str) {
    ...
    for filename in headers {
        let header = format!("{}/libs/{}/include/{}", 
                                     current_dir, api_type, filename);
        let prefix_name = filename.split(".").next().unwrap();
        generate_bindings(
            api_type,
            &header,
            &format!("{}/src/{}/generated/{}.rs", manifest_path, api_type, prefix_name)
...
fn generate_trader_api(api_type: &str, so_name: &str, orig_name: &str) {
    ...
    std::fs::copy(&format!("{}/libs/{}/linux64/{}", current_dir, api_type, orig_name), &target_so).expect("failed to copy so to outdir");
